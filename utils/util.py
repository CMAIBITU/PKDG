...
def calculate_hausdorff_distance(mask1, mask2):
    ...
    # 计算Hausdorff距离
    hausdorff_distance_1_2 = directed_hausdorff(points_mask1, points_mask2)[0]
    hausdorff_distance_2_1 = directed_hausdorff(points_mask2, points_mask1)[0]

    # 取两个方向的Hausdorff距离的最大值作为最终的Hausdorff距离
    hausdorff_distance = max(hausdorff_distance_1_2, hausdorff_distance_2_1)
...
def _eval_dice(gt_y, pred_y, detail=False):
    ...
    for cls in range(0,2):
        ...
        dice_this = 2*np.sum(gt*pred)/(np.sum(gt+pred))
...
def _eval_hd(gt_mask, pred_mask):
    hd = []

    for cls in range(2):  # 假设有两类

        gt = gt_mask[:, cls, ...]
        pred = pred_mask[:, cls, ...]

        # 计算Hausdorff距离
        hausdorff_distance = calculate_hausdorff_distance(gt, pred)
        hd.append(hausdorff_distance)

        # 输出Hausdorff距离
        #print("Class '{}', Hausdorff distance: {}".format(cls, hausdorff_distance))
...
#     asd = metric.binary.asd(pred, gt)
        ...
        # print (gt.shape)
        haus_cls = medpy.metric.binary.hd95(gt, (pred))
