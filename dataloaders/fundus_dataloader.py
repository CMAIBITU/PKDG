...
class Dataset(Dataset):
    ...
    def __init__(self, client_idx=None, freq_site_idx=None, split='train', transform=None):
        ...
        if split=='train':
            self.image_list = glob('/home/zll/fedDG/dataset/Fundus/{}/data_npy/*'.format(self.client_name[client_idx]))
            for i in range(len(self.client_name)):
                freq_list = glob('/home/zll/fedDG/dataset/Fundus/{}/freq_amp_npy/*'.format(self.client_name[i]))
    ...
    def __getitem__(self, idx):
        ...
        image_patch = raw_image
        #mask_patch = raw_mask



        im2 = np.where(raw_mask[..., :] == 0, 1, 0)

        im3 = np.where(raw_mask[..., :] != 255, 1, 0)
        ...
        for tar_freq_domain in np.random.choice(self.freq_site_index, 2):
            ...
            # np.concatenate ����ָ��axis�ϣ�ƴ���������� ��ԭʼ�������ֵ�������ƴ����һ��
            # image_patches ��� 384*384*9   Ӧ��*6 �Ŷ�ѽ����
            image_patches = np.concatenate([image_patches,image_patch_freq_1], axis=-1)
            #image_patches=np.array([image_patch,image_patch_freq_1])
...
def _get_coutour_sample(y_true):
    ...
    # �������ݵ�ά�� 384*384 *1
    return [disc_contour.transpose(0, 1, 2), disc_bg.transpose(0, 1, 2), cup_contour.transpose(0, 1, 2), cup_bg.transpose(0, 1, 2)]
